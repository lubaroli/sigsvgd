...
class PendulumModel(BaseModel):
    ...
    def __init__(self, g=9.8, mass=1.0, length=1.0, **kwargs):
        ...
        :key params_dist: A distribution to sample parameters for the forward
            model.
        :type kwargs: tuple or th.distributions.distribution.Distribution
        """
        ...
        self.__observation_space = Box(dim=2, low=-bounds, high=bounds, dtype=th.float)
        self.__action_space = Box(
            dim=1, low=-self.__max_torque, high=self.__max_torque, dtype=th.float,
        )
