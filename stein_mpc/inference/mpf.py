...
from ..inference.svgd import SVGD
from ..utils.math import bw_silverman
...
class MPF(SVGD):
    ...
    def update_prior(self, bw):
        ...
        mix = dist.Categorical(th.ones(n_particles))
        # TODO: check the best value for variance based on the bandwidth
        comp = dist.Independent(
            dist.MultivariateNormal(
                loc=self.x, covariance_matrix=bw ** 2 * th.eye(dim_particles),
            ),
    ...
    def phi(self, bw):
        ...
        grad_prior = th.autograd.grad(self.prior.log_prob(x).sum(), x)[0]
        ...
        grad_lik = th.autograd.grad(log_l.sum(), x)[0]
        score_func = grad_lik + grad_prior

        k_xx = self.kernel(x.flatten(1, -1), x.detach().clone().flatten(1, -1), bw=bw)
        grad_k = th.autograd.grad(k_xx.sum(), x)[0]

        phi = grad_k + th.tensordot(k_xx.detach(), score_func, dims=1) / x.size(0)
        return phi
