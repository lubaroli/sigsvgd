# TODO: Include control offset option
...
class DISCO(BaseController):
    ...
    def _compute_cost(self, states, actions, debug=False):
        ...
        if self._tf is not None:  # TODO: should we use log here?
            ...
            # TODO: should we average or logsumexp the cost over parameters?
        ...
        eps = th.add(actions[0], -self.a_seq)  # actions are equal over each param batch
        ctrl_costs = self.a_reg * th.tensordot(
    ...
    def forward(self, state, model, params_dist=None, ext_actions=None, debug=False):
        ...
        :rtype: (float, th.Tensor, th.Tensor, th.Tensor)
        """
        ...
        delta = th.tensordot(omega.exp().T, eps, dims=1)
